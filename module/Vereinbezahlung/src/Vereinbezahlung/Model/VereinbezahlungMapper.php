...
class VereinbezahlungMapper
{
    protected $tableName = 'verein_buchung';
    protected $dbAdapter;
    protected $sql;
    ...
    public function fetchAll()
    {
        ...
        $entityPrototype = new VereinbezahlungEntity();
    ...
    public function fetchAllgroup()
    {
        ...
        $select = $this->sql->select();
        $select->columns(array('Verwendungszweck','anzahlvereine' => new Expression('COUNT(id)'),'wertsum' => new Expression('SUM(wert)')))
    ...
    if ($id!=NULL & $useid!=NULL){
        $select = $this->sql->select();
        $select->where(array('Vereinid' => $id));
