<?php
namespace Athletenbezahlung\Model;
...
class AthletenbezahlungMapper
{
    ...
    public function athletbezahlungErstellen($id,$veranstaltungid,$athletid,$wert,$iban,$bic)
    {
        // TO-DO Fehlerabfragen
        ...
        $action = $this->dbAdapter->query('INSERT INTO betreiberkonto VALUES (?,?,?,?)', array($id,$wert,$date,$veranstaltungid));
    ...
    //statement erzeugen
    $action2 = $this->dbAdapter->query('SELECT * FROM `athlet_buchung` where id = ?', array($id));
    //ausführen des statements
    $resultstest=$action2->toarray();
